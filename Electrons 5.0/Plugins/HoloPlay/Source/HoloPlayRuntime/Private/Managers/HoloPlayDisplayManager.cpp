...
{
	if (FDisplayManagerHelper::GetInfoMessage(bIsServiceRunning))
	{
		CalibrationFound = 1;
		DisplayFound = 1;
	}
...
bool FHoloPlayDisplayManager::SetHoloPlayDisplaySettings(int32 ScreenIndex)
{
	...
	bIsServiceRunning = Loader.bIsServiceRunning;
	...
	if (Loader.CalibrationFound && Loader.DisplayFound)
	{
		...
		for (int32 i = 0; i < HoloPlayDisplayMetrics.LKGcount; i++)
		{
			...
			if (ScreenIndex == i)
			{
				Calibration = FHoloPlayDisplayMetrics::FCalibration(FDisplayManagerHelper::GetLKGcalIndex(i));
				TargetLookingGlass = i;
			}
		}
#if WITH_EDITOR
		if ( HoloPlayDisplayMetrics.LKGcount == 0 )
		{
		...
		if ( TargetLookingGlass > -1 )
		{
			...
			// Update settings of all AHoloPlayCapture objects
			if (World)
			{
				for (TActorIterator<AHoloPlayCapture> HoloPlayCaptureIt(World); HoloPlayCaptureIt; ++HoloPlayCaptureIt)
				{
					HoloPlayCaptureIt->UpdateTillingProperties();
				}
			}
		}
	...
	// Try to Load manually from the file
	else
	{
